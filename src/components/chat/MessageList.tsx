...
function getFriendlyToolLabel(toolName: string, args?: any): string {
  const cmd = args?.command;
  if (toolName === "str_replace_editor") {
    if (cmd === "create") return "Creating file";
    if (cmd === "str_replace" || cmd === "insert") return "Editing file";
    if (cmd === "view") return "Reading file";
    return "Editing file";
  }
                      ...
                      {message.parts.map((part, partIndex) => {
                        switch (part.type) {
                          ...
                          case "tool-invocation":
                            ...
                            const friendlyLabel = getFriendlyToolLabel(tool.toolName, tool.args);
                            return (
                              <div key={partIndex} className="inline-flex items-center gap-2 mt-2 px-3 py-1.5 bg-neutral-50 rounded-lg text-xs border border-neutral-200">
                                {tool.state === "result" && tool.result ? (
                                  <>
                                    ...
                                    <span className="text-neutral-700">{friendlyLabel}</span>
                                  </>
                                ) : (
                                  <>
